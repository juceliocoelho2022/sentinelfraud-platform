...
- Velocity checks atômicos no Redis por cliente, dispositivo e IP, usando janela móvel.
...
| FR004 | 5 ou mais transações do mesmo cliente em 1 minuto | 35 |
| FR005 | 5 ou mais transações do mesmo dispositivo em 1 minuto | 35 |
| FR006 | 5 ou mais transações do mesmo IP em 1 minuto | 35 |

Score menor que 40 aprova; de 40 a 69 envia para revisão; 70 ou mais bloqueia.

Os parâmetros das regras de velocidade podem ser alterados pelas variáveis
`VELOCITY_WINDOW`, `VELOCITY_THRESHOLD` e `VELOCITY_SCORE`. O contador usa um
Sorted Set e um script Lua executado atomicamente no Redis. O `transactionId`
é o membro do conjunto, evitando contagem duplicada da mesma transação. Os
identificadores usados nas chaves são protegidos com SHA-256 para não expor
cliente, dispositivo ou IP diretamente no Redis.
...
1. Device intelligence com adapter externo, circuit breaker, timeout e fallback.
2. Outbox transacional para garantir publicação Kafka sem dual-write.
3. Autenticação OAuth2/JWT, mTLS entre serviços e mascaramento de dados sensíveis.
4. OpenTelemetry com traces, logs correlacionados e SLO de latência p95.
5. Separação em serviços de decisão, feature store, casos e listas; contratos AsyncAPI.
6. AWS: API Gateway, EKS/ECS, MSK/SQS, ElastiCache, RDS/DynamoDB, S3 e IaC Terraform.
7. Shadow mode, feature flags, champion/challenger e modelo de ML versionado.
